...
router = APIRouter(tags=["Game Sessions"])  # Define router only once
...
async def create_game_session_endpoint(
    ...
    new_session = await game_session_crud.create_game_session(db, game_session_data)
    return {
...
async def get_all_game_sessions_endpoint(db: AsyncSession = Depends(get_db)):
    game_sessions = await game_session_crud.get_all_game_sessions(db)
...
async def get_game_session_by_id_endpoint(session_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def delete_game_session_by_id_endpoint(session_id: UUID, db: AsyncSession = Depends(get_db)):
